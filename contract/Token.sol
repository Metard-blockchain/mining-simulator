...
contract MetaHunterToken is Context, IBEP20, Ownable {
    ...
    function _safeTransfer(
        ...
        if (transferFeeRate > 0) {
            ...
            _transfer(sender, address(this), _fee);
            total_fee +=_fee;
            if (total_fee > 10000000000000000000) {
              uint256 trans = total_fee.mul(50).div(100);
              _transfer(address(this), addressReceiver, trans);
              uint256 _trans= total_fee - trans;
              _burn(address(this), _trans);
              total_fee = 0;
            }
    ...
    function swap (uint256 amount) public {
      ...
      uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount,24525898036735865, path, address(this), deadline);
    }
